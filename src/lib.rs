...
impl<T: Clone> Seat<T> {
    pub fn take(&self) -> T {
        ...
        if read + 1 == self.max {
            // no-one else will be cloning this value, so we can just take it instead of cloning it
            //
            // TODO
            // unfortunately, this requires having a mutable reference to the option, so we don't
            // do it for now.
        }
...
impl<T: Clone> Bus<T> {
    ...
    pub fn broadcast(&mut self, val: T) -> Result<(), T> {
